        ...
        <div class="file-results-shell">
                                ...
                                <td class="file-table-cell text-right">
                                    <div class="flex items-center justify-end gap-1">
                                        ...
                                        @if($file->status !== 'uploading' && $file->status !== 'deleted')
                                             <button type="button" @click="deleteFile({{ $file->id }})" class="file-action file-action-rose max-sm:h-8 max-sm:w-8" title="Hapus">
            ...
            <div x-show="viewMode === 'grid'" x-transition.opacity.duration.150ms x-cloak class="p-4">
                <div class="grid grid-cols-2 gap-3 sm:grid-cols-2 sm:gap-4 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
                    @forelse($files as $file)
                        <div class="file-grid-card !overflow-visible flex flex-col">
                            <div class="relative flex aspect-[4/3] items-center justify-center overflow-hidden rounded-t-lg bg-slate-50">
                            ...
                            <div class="flex items-start justify-between gap-2 border-t border-slate-100 bg-white px-2.5 py-2 sm:gap-3 sm:px-3">
                                <div class="min-w-0">
                                ...
                                <div x-data="{ open: false }" class="relative shrink-0" @click.outside="open = false">
                                    ...
                                    <div x-show="open" x-transition.origin.bottom.right x-cloak class="absolute bottom-full right-0 z-20 mb-1 w-32 overflow-hidden rounded-xl border border-slate-100 bg-white py-1 shadow-lg shadow-slate-950/10">
                                        ...
                                        @endif
                                        @if($file->status !== 'uploading' && $file->status !== 'deleted')
                                            <button type="button" @click="open = false; deleteFile({{ $file->id }})" class="flex w-full items-center gap-1.5 px-2.5 py-2 text-left text-xs font-medium text-slate-600 transition hover:bg-rose-50 hover:text-rose-600">
